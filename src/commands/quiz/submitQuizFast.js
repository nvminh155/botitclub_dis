...
const options = require("./variables/optionsSubmitQuizFast");
...
module.exports = {
  data: () => {
    const command = new SlashCommandBuilder()
      .setName("submit_quiz_fast")
      .setDescription("Nộp bài trắc nghiệm fast");
    options.forEach((op) => {
  ...
  async excute(params) {
    ...
    try {
      const [quizId, answer] = interaction.options._hoistedOptions.map(
        (op) => op.value
      );
      // console.log(quizId, answer);

      const quiz = await QuizModel.findOne({ quizId });
      const check_submitted = await QuizSubmission.findOne({
        userId: user.id,
        quizId,
      });
